#!/usr/bin/env python3
"""Orchestrate the ROS edge-markers pipeline in a single command."""
...
def run_workflow(
    *,
    ...
    # Configure simulation time so nodes follow rosbag clock
    ...
    print("[manager] launching edge marker pipeline")
    group.start("edge_markers", "roslaunch edge_detection edge_markers_from_cloud.launch", delay=launch_delay)

    if start_relay:
        relay_cmd = (
            "rosrun topic_tools relay "
            f"{shlex.quote(relay_input)} {shlex.quote(relay_output)}"
        )
        print("[manager] launching topic relay")
        group.start("topic_relay", relay_cmd, delay=launch_delay)
...
def main(
    *,
    ...
    start_relay: Optional[str] = None,
    relay_input: Optional[str] = None,
    relay_output: Optional[str] = None,
    launch_delay: Optional[str] = None,
) -> int:
    if any(v is None for v in (workspace_setup, bag_file)):
        parser = argparse.ArgumentParser(description="Launch the ROS edge marker workflow")
        ...
        parser.add_argument("--bag-file", required=True,
                            help="Bag file that includes RGB-D data with point clouds")
        parser.add_argument("--gripper-name", default="robotiq2f_140",
                            help="Override gripper_name argument for the display.launch file")
        ...
        parser.add_argument("--start-relay", action="store_true",
                            help="Start topic_tools relay /camera/depth_registered/points -> /edge_points")
        parser.add_argument("--relay-input", default="/camera/depth_registered/points",
                            help="Input topic for topic_tools relay (used when --start-relay is set)")
        parser.add_argument("--relay-output", default="/edge_points",
                            help="Output topic for topic_tools relay (used when --start-relay is set)")
        parser.add_argument("--launch-delay", type=float, default=3.0,
                            help="Delay in seconds between launching long-running processes")
    ...
    return run_workflow(
        ...
        relay_input=relay_input or "/camera/depth_registered/points",
        relay_output=relay_output or "/edge_points",
